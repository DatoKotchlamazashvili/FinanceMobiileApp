package com.example.financeapp.presentation.screen.create_password
...
data class CreatePasswordUiState(val password: String = "1276")

sealed interface CreatePasswordEvents {
    data class OnDialNumberClick(val number: Int) : CreatePasswordEvents
    data class OnDialIconClick(val callIconType: CallIconType) : CreatePasswordEvents
...
fun CreatePasswordScreenContent(
    ...
    onEvent: (CreatePasswordEvents) -> Unit,
    ...
    Column(modifier = modifier, horizontalAlignment = Alignment.CenterHorizontally) {
        ...
        FaCallDial(
            onNumberClicked = { onEvent(CreatePasswordEvents.OnDialNumberClick(it)) },
            onIconClicked = { onEvent(CreatePasswordEvents.OnDialIconClick(it)) }
        )
    }
